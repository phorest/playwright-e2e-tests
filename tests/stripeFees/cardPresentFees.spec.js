...
test("Verify Stripe fees applied to a card present transaction. @integratedPurchase @fees", async ({ page, request }) => {
  await page.goto('/');
  ...
  expect (paymentIntentResponseBody.charges.data[0].payment_method_details.card_present.brand).toBe('visa')
