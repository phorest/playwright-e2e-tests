...
test("Verify Stripe fees applied to a stored card sale. @integratedPurchase @fees", async ({ page, request }) => {
  // await page.goto('/');
  ...
  await page.locator(paySlideoverLocators.storedCardButton).click();
  ...
  expect(paymentIntentResponseBody.metadata.payment_channel).toBe('POS_STORED_CARD');
